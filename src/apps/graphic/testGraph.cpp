...
void drawRectGrid(SDL_Renderer *renderer) {
    ...
    for (int i = 0; i <= size + 1; ++i) {
        SDL_RenderDrawLine(renderer, start_x, start_y * i, start_x + 600,
                           start_y * i);
    }
    for (int i = 0; i <= size + 1; ++i) {
        SDL_RenderDrawLine(renderer, start_x * i, start_y, start_x * i,
                           start_y + 600);
    }
}
...
void drawHoverMouse(SDL_Renderer *renderer) {
    ...
    if (isOnGrid(x, y)) {
        SDL_SetRenderDrawColor(renderer, beige[0], beige[1], beige[2],
                               SDL_ALPHA_OPAQUE);
...
int main(int, char **) {
    if (SDL_Init(SDL_INIT_VIDEO) == 0) {
        ...
        if (SDL_CreateWindowAndRenderer(winW, winH, 0, &window, &renderer) ==
            0) {
