...
bool MenuHome::action(int choice, Parameters &param,
...
bool MenuBoard::action(int choice, Parameters &param,
                       std::shared_ptr<Menu> &current_menu) {
    if (choice % 2 == 1) {
        std::dynamic_pointer_cast<ParameterBoardSize>(
...
MenuPlayer::MenuPlayer(Color player_color, std::shared_ptr<Cursor> cursor)
    : Menu(cursor) {
...
bool MenuMonteCarlo::action(int choice, Parameters &param,
                            std::shared_ptr<Menu> &current_menu) {
    if (choice < 1)
        return false;
    std::string param_type = "PlayerWhite";
    if (m_player_color == Color::Black)
        param_type = "PlayerBlack";
    std::dynamic_pointer_cast<ParameterPlayer>(param.getParameter(param_type))
        ->setMonteCarloNbGame(choice);
    current_menu = std::make_shared<MenuHome>(m_cursor);
    return true;
}
