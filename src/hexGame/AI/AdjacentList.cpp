...
AdjacentList::AdjacentList(int max_size)
    : m_max_size(max_size), m_tab(new Tile *[max_size]), m_size(0) {
    for (int i = 0; i < m_max_size; ++i)
        m_tab[i] = nullptr;
}

AdjacentList::AdjacentList(const AdjacentList &adjList)
    : m_max_size(adjList.m_max_size), m_tab(new Tile *[m_max_size]),
      m_size(adjList.m_size) {
    for (int i = 0; i < m_max_size; ++i)
        m_tab[i] = adjList.m_tab[i];
}

AdjacentList::~AdjacentList() { delete m_tab; }
...
Tile *AdjacentList::operator[](int index) { return m_tab[index]; }
